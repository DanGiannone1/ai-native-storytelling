...
export const WelcomeSlide = ({ active }: WelcomeSlideProps) => {
  ...
  const titleText = "AI Native Storytelling"
      ...
      <div ref={containerRef} className="relative flex items-center justify-center h-screen">
        ...
        <div className="z-10 text-center px-8">
          ...
          <div className="flex justify-center mb-8">
            <div className="relative w-24 h-24">
              ...
              <div className="relative w-24 h-24 rounded-full bg-gradient-to-br from-cyan-500/20 to-violet-500/20 flex items-center justify-center shadow-[0_0_60px_-10px_rgba(34,211,238,0.4)]">
                ...
                <Icon name="Presentation" size={30} strokeWidth={1} className="text-cyan-300 relative z-10" />
              </div>
            </div>
          </div>

          {/* Main Title */}
          <h1 className="text-6xl md:text-7xl lg:text-8xl font-bold tracking-tight mb-6 text-white">
            {titleText.split('').map((char, i) => (
              <span
                key={i}
                className="title-char inline-block"
                style={{ opacity: Math.max(0.6, 1 - (i / (titleText.length * 2.5))) }}
              >
                {char === ' ' ? '\u00A0' : char}
              </span>
            ))}
          ...
          <p className="subtitle text-xl md:text-2xl text-white/40 font-light tracking-wide">
            A React-based presentation framework for stunning visual storytelling
          </p>
        </div>
      </div>
    </div>
